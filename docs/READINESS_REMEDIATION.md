...
| --- | --- | --- | --- | --- |
| `libreward-bridge/` was untracked in the source monorepo | Yes | Extracted the intended source, migrations, tests, lockfile, deployment, examples and docs into this public standalone repository while keeping secrets/build/database/wallet output ignored. | Standalone initial commit `9949c3b47f1d370a953d0d604a77bea602153765`. | Completed. |
| Standalone GitHub could not detect CI | Yes | Added the workflow at `.github/workflows/ci.yml`; retained `deployment/standalone-ci.yml` as a template mirror. | Static/local validation completed. A public standalone run requires push and is tracked in `docs/nlnet/SUBMISSION_CHECKLIST.md`. | Implemented; public run pending. |
| PostgreSQL had never been exercised | Yes | The source-monorepo audit used PostgreSQL 17.10, and the standalone recheck started a fresh isolated PostgreSQL 17.10 cluster from the portable distribution already on this host. | Empty migration and all 11 integration tests passed in the standalone checkout on 2026-07-14. | Completed locally; public CI run pending. |
| Concurrency/integration tests had been skipped | Yes | Added a required CI script and fixed defects exposed by real PostgreSQL. | The 2026-07-14 standalone run covered 50 concurrent creates, 50 concurrent claim starts, conflict detection, tenant isolation, operator RBAC, liquidity/daily limits, retention, signed webhook delivery and cross-worker serialization. | Completed locally; public CI run pending. |
...
The active standalone workflow runs these commands and performs the remaining container checks after push. Funded valueless evidence is recorded in `SANDBOX_EVIDENCE_2026-07-12.md`; real-money use remains prohibited until upstream risk is accepted and the named independent security/privacy/legal/accounting reviews approve it.
