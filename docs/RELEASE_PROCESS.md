...
4. Build with `npm pack --dry-run`; verify the allowlisted tarball contains runnable `dist/`, migrations, package metadata, notices, and no tests, environment files, grant drafts, or wallet material. Inspect checksums. Build the container from the commit and record its digest.
